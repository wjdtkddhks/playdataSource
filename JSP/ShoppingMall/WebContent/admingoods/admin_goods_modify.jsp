	...
	<tr>
		...
		<tr>
			...
			<select name="goods_category" size="1" >
				<option value="outwear" <%if(agb.getGOODS_CATEGORY().equals("outwerar")){%> selected <%} %>>아웃웨어</option>
				<option value="fulldress" <%if(agb.getGOODS_CATEGORY().equals("fulldress")){%> selected <%} %>>정장/신사복</option>
				<option value="tshirts" <%if(agb.getGOODS_CATEGORY().equals("tshirts")){%> selected <%} %>>티셔츠</option>
				<option value="shirts" <%if(agb.getGOODS_CATEGORY().equals("shirts")){%> selected <%} %>>와이셔츠</option>
				<option value="pants" <%if(agb.getGOODS_CATEGORY().equals("pants")){%> selected <%} %>>팬츠</option>
				<option value="shoes" <%if(agb.getGOODS_CATEGORY().equals("shoes")){%> selected <%} %>>슈즈</option>
			</select>
			</td>
		</tr>
		<tr>
