			...
			<div class='box'>
				...
				<div class='box-body'>
					<select name="searchType">
						...
						<option value="cw" <c:out value="${cri.searchType eq 'cw' ? 'selectd' : '' }" />>Content OR Writer</option>
						<option value="tcw" <c:out value="${cri.searchType eq 'tcw' ? 'selectd' : '' }" />>Title OR Content OR Writer</option>
 					</select>
 					<input type="text" name="keyword" id="kewyordInput" value="${cri.keyword }" >
	...
	$(document).ready(function(){
		$('#searchBtn').on("click", function(event){
			self.location = "list" + "${pageMaker.makeQuery(1)}"
								...
								+ "&keyword=" + encodeURIComponent($('#kewyordInput').val());
		});
