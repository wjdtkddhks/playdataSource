...
def get_user_timeline(client, keyword, language='ko', delimited=5000, filter_level='medium'):
    print('keyword:', keyword)
    base = 'https://stream.twitter.com/1.1'
    node = '/statuses/filter.json'
    fields = '?track=%s&langauge=%s&filter_level=%s&delimited=%d' % (keyword, language, filter_level, delimited)
...
def main():
    ...
    # screen_name = 'JTBC_news' #정보 원하는 계정 넣b기
    num_posts = 50
    jsonResult = []

    client = oauth2_request(CONSUMER_KEY, CONSUMER_SECRET, ACCESS_TOKEN, ACCESS_SECRET) #인증준비
    tweets = get_user_timeline(client, keyword) #토큰생성
    for i in tweets:
        getTwitterTwit(i, jsonResult)

    with open('%s_twitSearch.json' % keyword, 'w', encoding='utf-8') as outline:
        str_ = json.dumps(jsonResult, indent=4, sort_keys=True, ensure_ascii=False)
        outline.write(str_)

    print('%s.twitSearch.json Saved' % keyword)
