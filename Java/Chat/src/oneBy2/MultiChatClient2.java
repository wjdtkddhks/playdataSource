	...
	public void actionPerformed(ActionEvent arg0) {
		...
		if (obj == exitButton) {
			...
			if (rmsg[0].equals("귓속말")) {
				sendOne = new OnebyChat(ip, id, rmsg[1]);// rmsg[1] 받는 사람 id
				sendOne.connectServer(true);
			} else {
		...
		while (status) {
			try {
				...
				if (rmsg[1].equals("in")) {
					...
					takeOne = new OnebyChat(ip, id, rmsg[0]);
					takeOne.key = Integer.parseInt(rmsg[3]);
					takeOne.connectServer(false);
				} else {
